...
import { AddCollectionContainer, CollectionContainer, CollectionItem, CollectionItemLeft, ModalBody, ModalFooter } from "./collection-list.styles";
...
const CollectionList = () => {
  ...
  const { collections, removeCollectionFromList, updateCollectionName } =
  ...
  const submitCollectionName = () => {
    updateCollectionName({ name: collectionName }, newName);
    setModalOpen(false);
  ...
  const urlBanner = (collections && collections.length > 0 &&
    collections[0].animeList && collections[0].animeList[0])
  ...
  return (
    <CollectionContainer>
      ...
      <AddCollectionContainer>
        <Btn buttonType={'inverted'} onClick={() => setIsModalCollectionOpen(true)}>Add New Collection</Btn>
      </AddCollectionContainer>
      {collections &&
        collections.map((collection) => {
          const name = collection.name;
          const title = collection.name;
          const coverImage = collection.animeList && collection.animeList.length > 0 && collection.animeList[0]
      ...
      <Modal onClose={() => setModalOpen(false)} show={modalOpen}>
        ...
        <ModalFooter>
          <Btn onClick={submitCollectionName}>
            Confirm
          </Btn>
      ...
      <Modal onClose={() => setModalConfirmationOpen(false)} show={modalConfirmationOpen}>
        <h4 className="modal-title">Remove Collection</h4>
        <ModalBody>
          <span>Are You Sure Want to Delete item: {collectionName}</span>
        </ModalBody>
        <ModalFooter>
          <Btn onClick={() => confirmDelete(collectionName)}>
            Confirm
          </Btn>
          <Btn buttonType={'inverted'} onClick={() => setModalConfirmationOpen(false)}>
            Cancel
          </Btn>
        </ModalFooter>
      </Modal>

      <ModalCollection show={isModalCollectionOpen} onClose={() => setIsModalCollectionOpen(false)}/>
    </CollectionContainer>
  );
};
